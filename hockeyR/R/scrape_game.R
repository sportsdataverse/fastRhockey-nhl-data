...
scrape_game <- function(game_id) {
  ...
  site <- tryCatch(
    ...
    warning = function(cond) {
      message(paste0(
        "There was a problem with game ID ",
        game_id,
        "\n\n",
        cond
      ))
  ...
  pbp <- plays %>%
    ...
    dplyr::mutate(
      ...
      # clean up the times
      period_seconds = lubridate::period_to_seconds(lubridate::ms(
        timeInPeriod
      )),
      game_seconds = period_seconds + (1200 * (period - 1)),
      period_seconds_remaining = lubridate::period_to_seconds(lubridate::ms(
        timeRemaining
      )),
      ...
      event_player_3_id = dplyr::case_when(
        ...
        typeDescKey == "penalty" &
          is.na(committedByPlayerId) ~ servedByPlayerId,
      ...
      event_player_1_type = dplyr::case_when(
        ...
        typeDescKey %in%
          c("shot-on-goal", "missed-shot", "blocked-shot") ~ "Shooter",
      ...
      event_player_3_type = dplyr::case_when(
        ...
        typeDescKey == "goal" &
          is.na(assist2PlayerId) &
          !is.na(assist1PlayerId) ~ "Goalie",
        typeDescKey == "penalty" & is.na(committedByPlayerId) ~ "ServedBy",
        TRUE ~ NA_character_
      ),
      event_player_4_type = ifelse(
        !is.na(event_player_4_id),
        "Goalie",
        NA_character_
      ),
      # event goalie column
      ...
      x_fixed = dplyr::case_when(
        event_team_type == "home" & homeTeamDefendingSide == "right" ~ 0 -
          xCoord,
        event_team_type == "away" & homeTeamDefendingSide == "right" ~ 0 -
          xCoord,
        TRUE ~ xCoord
      ),
      ...
      empty_net = dplyr::case_when(
        event_team_id == home_id &
          away_goalie_in == 0 &
          event_type %in% corsi_events ~ TRUE,
    ...
    dplyr::left_join(
      teams %>%
        dplyr::select(event_team_abbr = event_team, event_team_id),
      by = "event_team_id"
    ) %>%
    # add event team name
    dplyr::left_join(
      team_logos_colors %>%
        dplyr::select(event_team = full_team_name, event_team_abbr = team_abbr),
      by = "event_team_abbr"
    ) %>%
    ...
    dplyr::left_join(
      rosters %>%
        dplyr::select(
          event_player_4_id = player_id,
          event_player_4_name = player_name
        ),
  ...
  if (!is.null(shifts)) {
    ...
    pbp_full <- pbp %>%
      ...
      dplyr::mutate(
        ...
        priority = 1 *
          ...
          6 *
            (event_type == "PERIOD_END" & !(period == 5 & season_type == "R")) +
      ...
      dplyr::mutate(
        home_index = as.numeric(cumsum(
          event_type == "CHANGE" &
            event_team_abbr == unique(pbp$home_abbr)
        )),
        away_index = as.numeric(cumsum(
          event_type == "CHANGE" &
    ...
    pbp_full <- pbp_full %>%
      ...
      dplyr::mutate(
        ...
        # last on-ice skater
        home_on_1 = ifelse(
          home_on_1 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_1
        ),
        home_on_2 = ifelse(
          home_on_2 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_2
        ),
        home_on_3 = ifelse(
          home_on_3 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_3
        ),
        home_on_4 = ifelse(
          home_on_4 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_4
        ),
        home_on_5 = ifelse(
          home_on_5 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_5
        ),
        home_on_6 = ifelse(
          home_on_6 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_6
        ),
        home_on_7 = ifelse(
          home_on_7 == home_goalie & !is.na(home_goalie),
          NA,
          home_on_7
        ),
        away_on_1 = ifelse(
          away_on_1 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_1
        ),
        away_on_2 = ifelse(
          away_on_2 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_2
        ),
        away_on_3 = ifelse(
          away_on_3 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_3
        ),
        away_on_4 = ifelse(
          away_on_4 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_4
        ),
        away_on_5 = ifelse(
          away_on_5 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_5
        ),
        away_on_6 = ifelse(
          away_on_6 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_6
        ),
        away_on_7 = ifelse(
          away_on_7 == away_goalie & !is.na(away_goalie),
          NA,
          away_on_7
        )
      ) %>%
      ...
      dplyr::rename_with(
        .cols = c(
          dplyr::starts_with("home_on"),
          dplyr::starts_with("away_on"),
          home_goalie,
          away_goalie
        ),
        ~ paste0(.x, "_id")
      ...
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_1 = player_name,
          home_on_1_id = player_id
        ),
        by = "home_on_1_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_2 = player_name,
          home_on_2_id = player_id
        ),
        by = "home_on_2_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_3 = player_name,
          home_on_3_id = player_id
        ),
        by = "home_on_3_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_4 = player_name,
          home_on_4_id = player_id
        ),
        by = "home_on_4_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_5 = player_name,
          home_on_5_id = player_id
        ),
        by = "home_on_5_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_6 = player_name,
          home_on_6_id = player_id
        ),
        by = "home_on_6_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          home_on_7 = player_name,
          home_on_7_id = player_id
        ),
        by = "home_on_7_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_1 = player_name,
          away_on_1_id = player_id
        ),
        by = "away_on_1_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_2 = player_name,
          away_on_2_id = player_id
        ),
        by = "away_on_2_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_3 = player_name,
          away_on_3_id = player_id
        ),
        by = "away_on_3_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_4 = player_name,
          away_on_4_id = player_id
        ),
        by = "away_on_4_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_5 = player_name,
          away_on_5_id = player_id
        ),
        by = "away_on_5_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_6 = player_name,
          away_on_6_id = player_id
        ),
        by = "away_on_6_id"
      ) %>%
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_on_7 = player_name,
          away_on_7_id = player_id
        ),
        by = "away_on_7_id"
      ) %>%
      dplyr::left_join(
      ...
      dplyr::left_join(
        dplyr::select(
          rosters,
          away_goalie = player_name,
          away_goalie_id = player_id
        ),
        by = "away_goalie_id"
      ) %>%
      tidyr::fill(
        c(home_skaters, away_skaters),
        .direction = "updown"
      ) %>%
      dplyr::mutate(
    ...
    pbp_full <- pbp_full %>%
      dplyr::mutate(
        ...
        description = dplyr::case_when(
          ...
          event_type == "PENALTY" & typeCode != "BEN" ~ glue::glue(
            "{event_player_1_name} {secondary_type} against {event_player_2_name}"
          ),
          event_type == "PENALTY" & typeCode == "BEN" ~ glue::glue(
            "{event_team_abbr} bench minor for {secondary_type}"
          ),
          event_type == "SHOT" ~ glue::glue(
            "{event_player_1_name} shot on goal saved by {event_goalie_name}"
          ),
          event_type == "STOP" & secondaryReason == "tv-timeout" ~ glue::glue(
            "Stoppage in play ({reason}) - TV timeout"
          ),
          event_type == "STOP" &
            secondaryReason ==
              "player-injury" ~ "Stoppage in play (player injury)",
          event_type == "STOP" &
            secondaryReason %not_in%
              c("tv-timeout", "player-injury") ~ glue::glue(
            "Stoppage in play ({reason})"
          ),
          event_type == "GOAL" & !is.na(event_player_4_id) ~ glue::glue(
            "{event_player_1_name} {secondary_type}, assists: {event_player_2_name}, {event_player_3_name}"
          ),
          event_type == "GOAL" &
            is.na(event_player_4_id) &
            !is.na(event_player_3_id) ~ glue::glue(
    ...
    # home_name/away_name are joined as columns later (~line 986), but are
    # needed here as scalars for case_when() comparisons against event_team
    home_name <- team_info$full_team_name[
      team_info$team_abbr == pbp$home_abbr[1]
    ][1]
    away_name <- team_info$full_team_name[
      team_info$team_abbr == pbp$away_abbr[1]
    ][1]
    ...
    pbp_full <- pbp %>%
      ...
      dplyr::mutate(
        med_x = stats::median(x[event_type %in% fenwick_events], na.rm = TRUE)
      ) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(
        x_fixed = dplyr::case_when(
    ...
    pbp_full <- pbp_full %>%
      dplyr::mutate(
        event_idx = dplyr::row_number() - 1,
        strength_code = NA_character_,
        strength = NA_character_,
        strength_state = NA_character_,
        period_type = dplyr::case_when(
          period < 4 ~ "REGULAR",
          season_type %in% c("R", "PR") & period == 4 ~ "OVERTIME",
          season_type %in% c("R", "PR") & period == 5 ~ "SHOOTOUT",
        ...
        description = dplyr::case_when(
          ...
          event_type == "STOP" &
            !is.na(secondaryReason) &
            secondaryReason == "tv-timeout" ~ glue::glue(
            "Stoppage in play ({reason}) - TV timeout"
          ),
          event_type == "STOP" &
            !is.na(secondaryReason) &
            secondaryReason ==
              "player-injury" ~ "Stoppage in play (player injury)",
          event_type == "STOP" & !is.na(secondaryReason) ~ glue::glue(
            "Stoppage in play ({reason})"
          ),
